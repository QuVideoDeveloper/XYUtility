...
+ (void)xy_saveVideo:(NSURL *)videoUrl toCustomAlbum:(NSString *)albumName  completionBlock:(XYPhotoLibrarySaveCompletionBlock)block {
    [self xy_saveAsset:videoUrl
               isVideo:YES
         toCustomAlbum:albumName
                   ext:[videoUrl.absoluteString pathExtension]
       completionBlock:^(NSURL * _Nonnull assetURL, NSError * _Nonnull error, NSString * _Nonnull localIdeitifier) {
        if (block) {
...
+ (void)xy_saveImage:(NSURL *)imageUrl
       toCustomAlbum:(NSString *)albumName
completionBlockWithLocalIdentifier:(XYPhotoLibrarySaveCompletionBlockWithLocalIdetifier)block {
    [self xy_saveAsset:imageUrl
               isVideo:NO
         toCustomAlbum:albumName
                   ext:[imageUrl.absoluteString pathExtension]
       completionBlock:block];
}
...
+ (void)xy_saveAsset:(NSURL *)fileUrl isVideo:(BOOL)isVideo toCustomAlbum:(NSString *)albumName ext:(NSString *)ext completionBlock:(XYPhotoLibrarySaveCompletionBlockWithLocalIdetifier)block {
    if (!fileUrl) {
        [self complete:block
              assetURL:nil
             errorCode:XYPhotoLibraryErrorInvalidFileUrl
         errorUserInfo:nil localIdeitifier:nil];
        return;
    }
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        if (albumName && albumName.length > 0) {
            ...
            if (!collectionRequest) {
                ...
                [self complete:block
                      assetURL:nil
                     errorCode:XYPhotoLibraryErrorCreateAssetFailed
                 errorUserInfo:nil
               localIdeitifier:nil];
                return;
            }
        ...
        if (!success || error) {
            ...
            [self complete:block
                  assetURL:nil
                 errorCode:XYPhotoLibraryErrorSaveAssetFailed
             errorUserInfo:nil
           localIdeitifier:nil];
            return;
        }
        ...
        if (!urlAsset) {
            [self complete:block
                  assetURL:nil
                 errorCode:XYPhotoLibraryErrorGetAssetUrlFailed
             errorUserInfo:nil
           localIdeitifier:nil];
        } else {
            [self complete:block
                  assetURL:urlAsset
                 errorCode:XYPhotoLibraryErrorNone
             errorUserInfo:nil
           localIdeitifier:localIdentifier];
        }
    }];
}
